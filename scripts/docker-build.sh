...
if [[ -z "$TARGET" ]]; then
    case "$GAME" in
        zh|zerohour)
            TARGET="z_generals z_worldbuilder z_w3dview z_mapcachebuilder z_imagepacker z_guiedit z_launcher z_datgen z_wdump"
            ;;
        generals)
            TARGET="g_generals g_worldbuilder g_w3dview g_mapcachebuilder g_imagepacker g_guiedit g_launcher g_datgen"
            ;;
        all)
            TARGET=""  # Build all
            ;;
